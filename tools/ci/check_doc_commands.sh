...
# .pre-commit-config.yaml (which the CI lint job mirrors via
# clang-format-20 / run-clang-tidy-20). If a doc literal disagrees with that rev,
# an agent installs the wrong toolchain and red-lines the exact CI gate the doc
# exists to prevent. This check fails the build so drift is caught at review time
# instead of a CI round-trip. Cyclone is a pure-CMake build (Bazel consumption is
# downstream-only), so only the clang-format/clang-tidy doc literals are checked.
#
...
# ---------------------------------------------------------------------------
# No line-number citations in living docs.
#
# `file.cpp:123` anchors rot silently: any commit that shifts lines in the
# cited file invalidates them with no error anywhere (it happened within hours
# of the 2026-07 doc refresh). Living docs must cite symbol anchors instead —
# a function/constant/comment-marker name that grep resolves and that renames
# loudly. Archived historical records are exempt (they describe the code as it
# was).
# ---------------------------------------------------------------------------
LIVING_DOCS=(README.md CONTRIBUTING.md CLAUDE.md AGENTS.md)
while IFS= read -r f; do
  LIVING_DOCS+=("$f")
done < <(find doc -maxdepth 1 -name '*.md' 2>/dev/null)
...
for DOC in "${LIVING_DOCS[@]}"; do
  ...
  while IFS=: read -r lineno match; do
    [ -z "$match" ] && continue
    echo "ERROR: $DOC:$lineno cites '$match' — line numbers rot; anchor to a symbol (function/constant/comment marker) instead" >&2
    CITE_HITS=1
  done < <(grep -noE '[A-Za-z0-9_./-]+\.(cpp|hpp|cc|h|txt|yml|yaml|json|sh):[0-9]+(-[0-9]+)?' "$DOC")
done
if [ "$CITE_HITS" -eq 0 ]; then
  echo "OK: no line-number citations in living docs."
else
  FATAL=1
fi
