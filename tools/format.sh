...
#
# CI enforces clang-format 20 in two places: the
# "clang-format check" step (find over src include/cyclone tests benchmarks
# examples) and the "pre-commit (config parity gate)" step, whose pinned
# mirrors-clang-format hook covers EVERY tracked C/C++ file — the broader,
# authoritative scope (it adds fuzz/). The pinned version is the
# mirrors-clang-format rev in .pre-commit-config.yaml. This script resolves a
# clang-format 20.x binary the same way and covers that full scope, so a local
# run matches CI exactly — no remote formatter box required.
#
...
# --- repo CI scope (union of tests.yml "clang-format check" and the
...
in_scope() {
  case "$1" in
    src/*|include/cyclone/*|tests/*|benchmarks/*|examples/*|fuzz/*)
      case "$1" in *.c|*.cpp|*.hpp|*.h|*.mm) return 0 ;; esac ;;
  esac
  return 1
}
