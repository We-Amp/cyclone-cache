...
# Small cold reads (issue #29): the round-6 1 Hz sampler plus one column,
# run beside every benchmark point until the file $1 exists.  Reads /proc
# and cgroup files only.
#
# Columns (counters are cumulative; the summariser takes deltas):
#   t_ms runner_jobs containers (docker-init processes: a CI job run in a
#     container, which the Runner.Worker count does not see)
...
while [ ! -e "$stop" ]; do
  ...
  echo "$(date +%s%3N) $(pgrep -c -x Runner.Worker) $(pgrep -c -x docker-init) $(vm) $cgs"
  sleep 1
done
