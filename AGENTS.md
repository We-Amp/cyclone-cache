...
Cyclone Cache — a C++23 disk-cache library (CMake) consumed by ModPageSpeed 1.1
and 2.0 via a downstream Bazel `git_repository`.
...
- **Style**: Google-based, enforced by `.clang-format` (`BasedOnStyle: Google`,
  `ColumnLimit: 80`). CI rejects non-conforming code via
  `clang-format-20 --dry-run --Werror`. LLVM 20 specifically — not 18/19/21.
- **Build/test like CI** (every job uses bundled SHA-256 + clang-20):
